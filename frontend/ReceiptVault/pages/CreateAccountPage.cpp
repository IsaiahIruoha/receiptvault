...
void CreateAccountPage::setupUI()
{
    ...
    createEmailEdit = new QLineEdit(this);
    ...
    connect(createAccountButton, &QPushButton::clicked, [this]() {
        ...
        if (!email.isEmpty() && !username.isEmpty() && !password.isEmpty() && password == confirmPassword) {
            emit accountCreationRequested(email, username, password);
        } else {
